// src/app/modules/PerformanceEdge/PerformanceDashboard/Reports/HighPerformersReport.jsx
import React from "react";
import { Card, CardHeader, CardTitle, CardContent } from "components/ui/card";
import { TableCustom } from "components";

export function HighPerformersReport({ dashboardData = {} }) {
  const { top_performers = [] } = dashboardData;
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>High Performer Identification</CardTitle>
        <p className="text-sm">
          Employees with consistently high performance ratings
        </p>
      </CardHeader>
      <CardContent>
        {top_performers.length === 0 ? (
          <div className="flex items-center justify-center h-32 text-neutral-800">
            <p>No high performers data available</p>
          </div>
        ) : (
          <TableCustom
            data={top_performers}
            columns={columns}
            pagination={true}
            dataTotalSize={top_performers.length}
          />
        )}
      </CardContent>
    </Card>
