...
import { CreateUpdateCycleForm, PerformanceCycleDetails } from "app/modules/PerformanceEdge";
import { toast } from "react-toastify";
import { deleteRecord } from "app/hooks/general";

const PerformanceCycleActions = ({ data, reloadData = () => { }, DataList = [] }) => {
    ...
    const confirmDelete = async () => {
        try {
            await deleteRecord(`/cycles/${data.id}`, `${data.name}`);
    ...
    return (
        <>
            <DropdownActionMenu
                ...
                viewText="View Cycle"
                editText="Edit Cycle"
                deleteText="Delete Cycle"
                menuTooltip="Form Cycle"
            />

            {deleteDurationState?.open && (
                <AlertDialogue
                    title="Confirm Delete?"
                    description={`This action can't be undone. All information associated with "${data?.nme}" will be lost.`}
...
export default PerformanceCycleActions;
