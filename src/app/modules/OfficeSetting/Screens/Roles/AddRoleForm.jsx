...
const AddRoleForm = ({ isOpen, setIsOpen, edit, reload }) => {
  ...
  useEffect(() => {
    ...
    const filtered = permissionsSchema.map(module => {
      // Check if module name matches
      const moduleMatches = module.name.toLowerCase().includes(searchTermLower);
      
      // Filter and map submodules
      const filteredSubmodules = module.subModules?.map(subModule => {
        // Check if submodule name matches
        const subModuleMatches = subModule.name.toLowerCase().includes(searchTermLower);
      ...
      return moduleMatches || filteredSubmodules.length > 0 || filteredFeatures.length > 0
        ? { 
            ...module, 
  ...
  const validateForm = async (values) => {
    ...
    if (!values.name || values.name.trim() === '') {
      ...
      try {
        ...
        if (!isUnique) {
          errors.name = 'Role Name already exists. Please choose a different name';
        }
      } catch (error) {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      // Show error message
      const errorMessage = error?.response?.data?.message || `Failed to ${isEditMode ? "update" : "add"} role.`;
