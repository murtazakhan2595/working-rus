...
const Calendar = ({ baseUrl, token }) => {
  ...
  function ServerDay(props) {
    ...
    return (
        <PickersDay {...other} outsideCurrentMonth={outsideCurrentMonth} day={day} sx={{bgcolor:day.day() === 0 ?  "#FECACA" :"" ,color: day.day() === 0 ?  "#A82822" :""  }} />
    ); 
  }
  ...
  return (
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs} locale="en" >
      ...
      showDaysOutsideCurrentMonth value={selectedDate} onChange={(newValue) => {handleDateClick(newValue)}}/>
      </div>
...
<div className="text-xs opacity-60">{selectedDate?.format('MMMM D, YYYY')}</div>
</div>
        <div className="divide-y scroll max-h-56 overflow-y-auto">
          {(allFilterdMeetings.length > 0 && !msg) ? allFilterdMeetings?.map((meeting,index)=>(
            ...
            <div className="flex flex-col border-l-8 border-[#25A8E0] rounded-s-md pl-2 gap-1 py-2">
              ...
              <div className="text-xs opacity-60">3:30pm - 4:00</div>
            </div>
          </div>
          )): <div className="flex justify-center items-center opacity-50 text-sm">{msg}</div>}
        </div>
      </div>
