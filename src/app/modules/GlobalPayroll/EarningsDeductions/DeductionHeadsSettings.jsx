...
import { TrendingDown, Plus, Edit2, Trash2, Save } from "lucide-react";
import { Card } from "components/ui/card";
import { Button } from "components/ui/button";
import { Label } from "components/ui/label";
import { Switch } from "components/ui/switch";

const DeductionHeadsSettings = () => {
  ...
  const [currentHead, setCurrentHead] = useState({
    ...
    maxAmount: null,
    isRecurring: true,
    displayOrder: deductionHeads.length + 1,
    enabled: true,
  });
  ...
  const handleSave = () => {
    if (currentHead.id) {
      setDeductionHeads(
        deductionHeads.map((h) => (h.id === currentHead.id ? currentHead : h))
      );
    } else {
      setDeductionHeads([...deductionHeads, { ...currentHead, id: Date.now() }]);
    }
    setIsEditing(false);
    setCurrentHead({
      name: "",
      ...
      maxAmount: null,
      isRecurring: true,
      displayOrder: deductionHeads.length + 2,
      enabled: true,
    });
  };
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this deduction head?")) {
      setDeductionHeads(deductionHeads.filter((h) => h.id !== id));
    }
  };

  const handleToggle = (id) => {
    setDeductionHeads(
      deductionHeads.map((h) => (h.id === id ? { ...h, enabled: !h.enabled } : h))
    );
  };
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Deduction Heads Configuration</h1>
          <p className="text-gray-600 mt-1">
            Configure deductions like Tax, PF, Loans, Absences, etc.
          </p>
        </div>
        <Button
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="text-sm">
                  ...
                  {head.percentage && (
                    <span className="ml-1 text-red-600">({head.percentage}%)</span>
                  )}
                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-xl font-bold text-gray-900 mb-4">
              {currentHead.id ? "Edit Deduction Head" : "Add New Deduction Head"}
            </h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Per Day">Per Day Deduction</option>
...
export default DeductionHeadsSettings;
