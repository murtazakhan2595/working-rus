          ...
          formatter: (cell) => {
            return (
              <ol className="[list-style:decimal-leading-zero] ml-5">
                {(cell || []).map((rotation) => {
                  const approver = (rotation?.approval_logs[0] || {})
                    ?.changed_by;
                  return (
                    <li
                      className={
                        "marker:text-plum-900 marker:font-semibold  mb-3"
                      }
                    >
                      <div>
                        <span className="text-plum-900 font-semibold">
                          Rotation{" "}
                        </span>
                        (
                        ...
                        {approver && (
                          <span>
                            {" "}
                            by <EmployeeName value={approver} />
                          </span>
                        )}
                        )
                      </div>
                      <ul className="[list-style:disc] ml-4">
                        {rotation.new_branch && (
                          <li>
                            <BranchName value={rotation.old_branch} /> (Branch){" "}
                            {"->"} <BranchName value={rotation.new_branch} />{" "}
                            (Branch)
                          </li>
                        )}
                        {rotation.new_department && (
                          <li>
                            <DepartmentName value={rotation.old_department} />{" "}
                            (Department) {"->"}{" "}
                            <DepartmentName value={rotation.new_department} />{" "}
                            (Department)
                          </li>
                        )}
                        {rotation.new_designation && (
                          <li>
                            <DesignationName value={rotation.old_designation} />{" "}
                            (Designation) {"->"}{" "}
                            <DesignationName value={rotation.new_designation} />{" "}
                            (Designation)
                          </li>
                        )}
                        {rotation.new_reporting_manager && (
                          <li>
                            <ManagerName
                              value={rotation.old_reporting_manager}
                            />{" "}
                            (Manager) {"->"}{" "}
                            <ManagerName
                              value={rotation.new_reporting_manager}
                            />{" "}
                            (Manager)
                          </li>
                        )}
                      ...
                      <div>
                        Effective from {renderDate(rotation.effective_date)}{" "}
                        {rotation.rotation_type === "temporary" &&
                          ` to ${renderDate(rotation.rotation_expiry_date)}`}
                      </div>
                    </li>
                  );
                })}
  ...
  const fetchData = async (id, isMounted) => {
    try {
      ...
      if (isMounted) {
        ...
        const rotation_summary = await getJobRotationRequests({
