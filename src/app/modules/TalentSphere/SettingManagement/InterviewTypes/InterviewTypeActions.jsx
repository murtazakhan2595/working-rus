...
import {AddUpdateInterviewTypeForm} from "app/modules/TalentSphere";
...
const InterviewTypeActions = ({ data, DataList = [], reloadData = () => { } }) => {
    ...
    const confirmDelete = async () => {
        try {
            await deleteRecord(`/interview-types/${data.id}`, `${data.name}`);
            setDeleteForm(null);
            reloadData(true);
        } catch (error) {
    ...
    return (
        <>
            <DropdownActionMenu
                ...
                viewText="View Interview Type"
                editText="Edit Interview Type"
                deleteText="Delete Interview Type"
                menuTooltip="Interview Type Actions"
            />
...
export default InterviewTypeActions;
