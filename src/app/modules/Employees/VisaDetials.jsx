...
const VisaDetials = ({ prevstep, nextstep }) => {
    ...
    //     living_country_id_no: Joi.string().required().label('Living Country ID Number'),
    //     place_of_issuance: Joi.string().required().label('Place of Issuance'),
    ...
    // // Define schema for visa details
    // const visaSchema = Joi.object({
    //     entry_permit_number: Joi.string().label('Entry Permit Number'),
    //     country_of_visa_issuance: Joi.string().label('Visa Issuance Country'),
    //     uid_number: Joi.string().label('UID Number'),
    //     visa_type: Joi.string().label('Visa Type'),
    //     visa_issuance_date: Joi.date().label('Visa Issuance Date'),
    //     visa_expiry_date: Joi.date().label('Visa Expiry Date'),
    //     visa_duration: Joi.string().label('Visa Duration'),
    //     visa_country_entry_date: Joi.date().label('Visa Country Entry Date'),
    //     visa_country_exit_date: Joi.date().label('Visa Country Exit Date'),
    // });

    // // Define schema for insurance details
    // const insuranceSchema = Joi.object({
    //     dha_id: Joi.string().label('DHA ID'),
    //     card_number: Joi.string().label('Card Number'),
    //     insurance_policy: Joi.string().label('Insurance Policy'),
    //     insurance_company: Joi.string().label('Insurance Company'),
    //     insurance_active_date: Joi.date().label('Insurance Active Date'),
    //     insurance_expiry_date: Joi.date().label('Insurance Expiry Date'),
    // });
    ...
    const handleNextStep = (e) => {
        ...
        // // Check if the showPassport checkbox is checked
        // if (showPassport) {
        //     validationSchema = passportSchema;
        // } else {
        //     // If not checked, use an empty schema to bypass validation
        //     validationSchema = Joi.object({});
        // }

        // // Validate all passport details
        // const { error } = validationSchema.validate(visaDetails, { abortEarly: false });

        // if (error) {
        //     console.error(error.details.map((err) => err.message));
        // } else {
        // No validation errors, proceed with your logic
        nextstep(); // For example, move to the next step
        // }
    };
                ...
                {showPassport &&
                    <div className='flex w-[100%] flex-wrap items-center gap-x-5'>
                        <div className="flex flex-col gap-y-1 w-[100%] lg:w-[20%]">
                            ...
                            <input type="text" name="passport_number" required={showPassport ? true : false} value={visaDetails.passport_number} onChange={(e) => handleChange(e.target.name, e.target.value)} placeholder="Passport Number" className="pl-2 bg-white rounded h-8 text-sm placeholder-[#555657] placeholder-opacity-50 w-[100%]"
                            />
                        </div>
                        <div className="flex flex-col gap-y-1 w-[100%] lg:w-[20%]">
                            <label
                            ...
                            <Select
                                ...
                                required={showPassport ? true : false}
                            />
                        </div>
