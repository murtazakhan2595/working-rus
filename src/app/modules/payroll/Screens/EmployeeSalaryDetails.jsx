...
import { DollarSign, TrendingUp, Calendar, ArrowLeft, CalendarIcon, Filter } from "lucide-react"
...
export default function EmployeeSalaryDetails() {
  ...
  return (
    <div className="container p-4 mx-auto">
      <div className="mb-4">
        <Button variant="ghost" onClick={handleBack} className="p-4 text-xl text-balance">
        ...
        <Card>
          <CardContent className="flex items-center pt-6 space-x-4">
            <Avatar className="w-20 h-20 ">
              ...
              <AvatarFallback className="bg-plum-400">{employeeData.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
            </Avatar>
            <div>
              <p className="text-base text-black">{employeeData.id}</p>
              <h2 className="text-2xl font-bold text-plum-900">{employeeData.name}</h2>
              <p className="text-base text-muted-foreground">{employeeData.role}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-base font-medium text-black">Cost to Company</CardTitle>
            <DollarSign className="w-4 h-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-plum-900">{employeeData.costToCompany} AED</div>
            <p className="text-xs text-muted-foreground">{employeeData.costToCompanyWords}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            <CardTitle className="text-base font-medium text-black">No. of increments</CardTitle>
            <TrendingUp className="w-4 h-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-plum-900">{employeeData.incrementsCount}</div>
            <p className="text-xs text-muted-foreground">Last increment {employeeData.lastIncrementDate}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 mb-4 md:grid-cols-3 h-[450px]">
        <Card className="h-fit">
          ...
          <CardContent>
           <h3 className="mb-4 text-lg font-semibold text-black">CTC Components</h3>
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-row font-bold">
                <div className="flex-1 mb-4 text-lg font-medium text-black">Total Salary in AED</div>
                <div className="flex-1 text-right">
                  AED {salaryBreakup.reduce((total, item) => total + parseFloat(item.amount), 0).toFixed(2)}
                </div>
              </div>
            </div>
            ...
            {['Approved', 'Pending', 'Rejected'].map((status, index) => (
              <div key={index}>
                <div className="flex flex-row items-center justify-between pb-2 space-y-0">
                  <div className="text-base font-medium text-black">{status} Revision</div>
                </div>
                <div>
                  <div className={`text-2xl font-bold text-plum-900 ${status === 'Rejected' ? 'text-red-600' : ''}`}>
                    {salaryRevisions.filter(revision => revision.status === status).length}
                  </div>
                </div>
              </div>
            ))}
            ...
            <TableBody>
              {salaryRevisions.map((revision, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <span className={`bg-${revision.letterStatus === 'Issued' ? 'green' : revision.letterStatus === 'Not Issued' ? 'red' : 'blue'}-100 text-${revision.letterStatus === 'Issued' ? 'green' : revision.letterStatus === 'Not Issued' ? 'red' : 'blue'}-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded`}>
...
function SalarySummary({
  ...
  return (
    <Card className="w-full ">
      <CardHeader>
        <CardTitle className="text-2xl font-semibold text-plum-900">Salary Summary</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <div className="grid grid-cols-2 gap-y-4">
          ...
          <div className="space-y-4">
            <div className="flex flex-row gap-6">
              ...
              <p className="font-medium text-black">{dummyData.lastRevisedDate}</p>
            </div>
            <div className="flex flex-row gap-6">
