        ...
        <div className="p-2 space-y-4">
            ...
            <div className="p-4 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <h3 className="text-lg font-semibold text-slate-1200 text-balance">Employee Pay Summary</h3>
                  <div className="space-y-2">
                    {[
                      { label: "ID", value: <EmployeeID value={employeeID} /> },
                      { label: "Name", value: `${employeeData?.first_name} ${employeeData?.last_name}` },
                      { label: "Designation", value: <DesignationName value={employeeData?.department_position} /> },
                      { label: "Date of Joining", value: moment(employeeData?.joining_date).format("DD/MM/YYYY") },
                      { label: "Pay Period", value: moment(payslip?.generated_at).format("MMMM YYYY") },
                      { label: "Pay Date", value: moment(payslip?.generated_at).format("DD/MM/YYYY") }
                    ].map((item, index) => (
                      <div key={index} className="flex justify-between py-1 border-0">
                ...
                <Card className="flex flex-col justify-between border rounded-lg shadow-none border-color-[#D0CDD7]">
                  ...
                  <CardFooter className="flex flex-row justify-between w-full bg-[#FEFCFF] border-t p-0 rounded-b-lg">
                    <div className="flex flex-row items-center justify-center flex-1 gap-1 p-2">
                      <p className="font-semibold text-normal text-slate-1200">20</p>
                      {/* {employeeData.daysOfWork} */}
                      {/* <p className="font-semibold text-normal text-slate-1200"> {employeeData.daysOfWork}</p> */}
                      <p className="text-normal text-slate-1000">Days of work</p>
                    </div>
                    <div className="flex flex-row items-center justify-center flex-1 gap-1 p-2">
                      <p className="font-semibold text-normal text-slate-1200">11</p>
              ...
              <Card className="border-color-[#D0CDD7]">
                <CardContent className="grid grid-cols-2 gap-6 p-6">
                  ...
                  {/* Employee Earnings */}
                  <div>
                    <h3 className="mb-4 text-lg font-semibold text-slate-1200">Employee Earnings</h3>
                    <div className="flex flex-col min-h-[260px]">
                      {/* Header - Always visible */}
                      <div className="flex justify-between p-4 border-b">
                        <div className="font-medium text-slate-1000">Earning types</div>
                        <div className="font-medium text-right text-slate-1000">Amount</div>
                      </div>

                      {/* Body - Flexible space */}
                      <div className="flex-1">
                        {payslip?.total_earning_types?.map((item, index) => (
                          <div key={index} className="flex justify-between px-4 border-b last:border-b-0">
                            <div className="py-3 capitalize">{item.description}</div>
                            <div className="py-3">AED {Number(item.amount).toFixed(2)}</div>
                          </div>
                        ))}
                      </div>
                  ...
                  {/* Employee Deductions */}
                  <div className="">
                    <h3 className="mb-4 text-lg font-semibold text-slate-1200">Employee Deductions</h3>
                    <div className="flex flex-col min-h-[260px]">
                      {/* Header - Always visible */}
                      <div className="flex justify-between p-4 border-b">
                        <div className="font-medium text-slate-1000">Deduction types</div>
                        <div className="font-medium text-right text-slate-1000">Amount</div>
                      </div>
                      ...
                      <div className="flex justify-between p-4 mt-auto bg-plum-200">
                        <div className="text-lg font-bold">Total in AED</div>
                        <div className="text-lg font-bold">
                          AED {Number(payslip?.gross_salary - payslip?.net_salary || 0).toFixed(2)}
                        </div>
                      </div>
                    </div>
                    <div className="mt-2 text-sm text-center text-slate-1100">
              ...
              {isEos && (
                <Card className="border-color-[#D0CDD7]">
                  <CardHeader>
                    <CardTitle> <h3 className="mb-4 text-lg font-semibold text-slate-1200">End Of Service</h3></CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="w-full">
                      ...
                      <div className="max-h-[320px] overflow-auto">
                        {eosData?.map((item, index) => (
                          <div key={index} className="flex justify-between border-b last:border-b-0">
              ...
              <Card className="border-color-[#D0CDD7]">
                <CardHeader>
                  <CardTitle> <h3 className="mb-4 text-lg font-semibold text-slate-1200">Net Pay</h3></CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="w-full">
                    {/* Header */}
              ...
              <div className="grid grid-cols-2 gap-0 rounded-lg shadow-none border  border-color-[#D0CDD7]">
                <Card className="border-0 rounded-lg  p-2 shadow-none border-r border-color-[#D0CDD7] rounded-r-none" >
                  <CardHeader className="p-0">
                    <CardTitle> <h3 className="mb-4 text-lg font-semibold text-slate-1200">Comments</h3></CardTitle>
                  </CardHeader>
                ...
                <Card className="p-2 border-0 rounded-lg rounded-l-none shadow-none">
                  <CardHeader className="p-0">
                    <CardTitle> <h3 className="mb-4 text-lg font-semibold text-slate-1200">Signature</h3></CardTitle>
                  </CardHeader>
                  <CardContent className="p-0">
                    <div className=""></div>
                  </CardContent>
              ...
              <div className="flex justify-end mt-4">
                <p className="text-sm italic text-slate-800">
                  This pay slip is computer-generated and does not require a physical signature. All information contained within this document is accurate to the best of our knowledge at the time of generation. However, it is provided “as is” without any warranties, express or implied. The employer assumes no responsibility for any errors or omissions. For any discrepancies or questions, please contact the HR department
                </p>
              </div>
            </div>
          )}
          ...
          {payslip && (
            <CardFooter className="flex justify-end py-2 no-print">
              <Button onClick={() => toPDF()} className="px-3 py-1">
                Download PDF
              </Button>
            </CardFooter>
          )}
