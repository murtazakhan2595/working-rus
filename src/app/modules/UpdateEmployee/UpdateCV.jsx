...
const SubmitCV = ({ errors, setErrors, prevstep, nextstep, substep ,  token,
  userProfile,
  baseUrl,}) => {
...
const fetchData = async () => {
  try {
    const cvResponse = await axios.get(`${baseUrl}/attachment/?search={"employee_id":${id},"name":"cv"}`, {
      headers,
    });
    ...
    if (!cv) {
      ...
      try {
        if (haveCV){
          ...
          const cvResponse = await axios.patch(`${baseUrl}/attachment/${haveCV.id}`,
          {
            document: cv
          } ,{
            headers,
          ...
          const cvResponse = await axios.post(`${baseUrl}/attachment`,  {
            employee_id: id,
            name: "cv",
        ...
        <div className='flex flex-col md:flex-row lg:gap-x-36'>
          <div className='order-2 md:order-1 md:w-[65%]'>
            ...
            rounded-lg py-1 text-input">
              ...
              <div className='bg-gray-200 border-gray-400 border py-1 px-3 rounded-l-md '>{isEdit ? "Upload CV" : "CV"} </div> 
              <div className='py-1 px-3 border-gray-200 border rounded-r-md'>{cvName}</div> 
              </div>
              <input id="file-upload" disabled={isEdit ? false : true} type="file" name="cv" accept=".doc, .docx"
                max-size="104857600" className='hidden' onChange={handleFileChange} />
            </label>
            {errors.cv && <small className='text-red-500 block'>{errors.cv}</small>}
            <small className='text-gray-400'>Upload a doc or docx file and no larger than 1 MB.</small>
          </div>
        </div>
