...
export const getCurrencyList = async (payload) => {
  const pageNo = payload?.options?.page ?? "";
  const pageSize = payload?.options?.sizePerPage ?? "";
  const filterData = payload?.filterData ?? {};
  const ordering = payload?.ordering ?? "id";
  const URL = `/currencies/?${ordering ? `ordering=${ordering}&` : ""}${pageNo ? `page=${pageNo}&` : ""
    }${pageSize ? `page_size=${pageSize}&` : ""}search=${encodeURIComponent(
      JSON.stringify(filterData)
    )}`;
  try {
    ...
    if (response.status === 200) {
      const ResponseData = response.data;
      const ResponseDataList = await mapCurrencyList(ResponseData.results);
      return { results: ResponseDataList, count: ResponseData.count };
    }
  } catch (error) {
    console.error("Error getting regions list:", error);
...
export const getCurrencyData = async (id) => {
  try {
    const response = await axios.get(`${baseUrl}/currencies/${id}`, {
      headers: headers(),
    });
    if (response.status === 200) {
      const ResponseData = mapCurrencyData(response.data);
      return ResponseData;
    }
  } catch (error) {
    console.error("Error getting onboarding document by id:", error);
...
export const saveUpdateCurrency = async (payload, id) => {
  try {
    const url = id
    ...
    const method = id ? "PATCH" : "POST"; // Determine method based on existence of id
    const expectedStatus = id ? 200 : 201;
    const finalPayload = mapCurrencyPayloadData(payload);
    const response = await axios({
      method,
      url,
      data: finalPayload,
