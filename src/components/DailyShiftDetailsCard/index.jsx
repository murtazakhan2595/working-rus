      ...
      <CardContent>
        <div className="space-y-4">
          {Object.entries(shiftDetails).map(([day, detail]) => (
            <div 
              key={day} 
              className={`p-4 rounded-lg border transition-all duration-200 hover:shadow-md ${dayStyles[day].border} ${dayStyles[day].bg}`}
            >
              <h3 className={`flex items-center gap-2 font-medium capitalize mb-3 ${dayStyles[day].title}`}>
              ...
              {detail.status === 'active' ? (
                <div className="space-y-2">
                  {detail.shifts.map((shift, index) => (
                    <div 
                      key={index} 
                      className={`flex items-center justify-between px-3 py-2 rounded-md bg-white/80 ${dayStyles[day].text}`}
                    >
                      <span className="font-medium">{shift.start}</span>
                      <ArrowRight className={`h-4 w-4 ${dayStyles[day].icon}`} />
                      <span className="font-medium">{shift.end}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <div className={`text-center py-2 px-3 rounded-md bg-white/80 font-medium ${dayStyles[day].text}`}>
                  {detail.status}
                </div>
              )}
            </div>
